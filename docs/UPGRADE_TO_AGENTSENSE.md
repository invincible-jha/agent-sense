...
```bash
pip install "agent-sense[agentcore]"
```

### 2. Swap the configuration backend

Replace the default ``Settings`` class with the enterprise version
which reads from your AgentSense control plane:

```python
# Before (open-source)
from agent_sense.core import Settings
...
The AgentSense SDK ships with a full set of enterprise plugins that
auto-register when the package is installed:

```python
from agent_sense.plugins.registry import processor_registry
processor_registry.load_entrypoints("agent_sense.plugins")
```
