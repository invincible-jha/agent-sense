# Architecture — agent-sense

## Overview

Human-agent interaction SDK with accessibility and dialogue management

This document describes the high-level architecture of agent-sense
...
agent-sense uses a decorator-based plugin registry backed by
``importlib.metadata`` entry-points. This allows third-party packages
(including the AgentSense enterprise edition) to extend the system
...
from agent_sense.core import BaseProcessor  # example base class
